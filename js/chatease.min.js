chatease=function(){if(chatease.api)return chatease.api.getInstance.apply(this,arguments)};chatease.version="1.1.00";
(function(c){function e(a){return null!==navigator.userAgent.toLowerCase().match(a)}var f=c.utils={};f.exists=function(a){switch(f.typeOf(a)){case "string":return 0<a.length;case "object":return null!==a;case "null":return!1}return!0};f.extend=function(){var a=Array.prototype.slice.call(arguments,0),b=a[0];if(1<a.length)for(var d=1;d<a.length;d++)f.foreach(a[d],function(a,d){void 0!==d&&null!==d&&(b[a]=d)});return b};f.foreach=function(a,b){for(var d in a)a.hasOwnProperty&&"function"===f.typeOf(a.hasOwnProperty)?
a.hasOwnProperty(d)&&b(d,a[d]):b(d,a[d])};f.getCookie=function(a){var b;return(b=document.cookie.match(new RegExp("(^| )"+a+"=([^;]*)(;|$)")))?unescape(b[2]):null};f.formatTime=function(a){var b=a.getHours()+1,d=a.getMinutes(),g=a.getSeconds();return a.toLocaleDateString()+" "+f.pad(b,2)+":"+f.pad(d,2)+":"+f.pad(g,2)};f.pad=function(a,b){for(a+="";a.length<b;)a="0"+a;return a};f.typeOf=function(a){if(null===a||void 0===a)return"null";var b=typeof a;if("object"===b)try{var d=a.toString().match(/^\[object ([a-z]+)\]$/i);
if(d&&1<d.length&&d[1])return d[1].toLowerCase()}catch(g){}return b};f.isInt=function(a){return 0===parseFloat(a)%1};f.trim=function(a){return a.replace(/^\s+|\s+$/g,"")};f.indexOf=function(a,b){if(null==a)return-1;for(var d=0;d<a.length;d++)if(a[d]===b)return d;return-1};f.createElement=function(a,b){a=document.createElement(a);b&&(a.className=b);return a};f.addClass=function(a,b){var d="string"===f.typeOf(a.className)?a.className.split(" "):[];b="array"===f.typeOf(b)?b:b.split(" ");f.foreach(b,
function(a,b){-1===f.indexOf(d,b)&&d.push(b)});a.className=f.trim(d.join(" "))};f.removeClass=function(a,b){var d="string"===f.typeOf(a.className)?a.className.split(" "):[];b="array"===f.typeOf(b)?b:b.split(" ");f.foreach(b,function(a,b){a=f.indexOf(d,b);0<=a&&d.splice(a,1)});a.className=f.trim(d.join(" "))};f.emptyElement=function(a){for(;a.firstChild;)a.removeChild(a.firstChild)};f.isMSIE=function(a){return e(new RegExp("msie\\s*"+(a||""),"i"))};f.isSafari=function(){return e(/safari/i)&&!e(/chrome/i)&&
!e(/chromium/i)&&!e(/android/i)};f.isIOS=function(a){return a?e(new RegExp("iP(hone|ad|od).+\\sOS\\s"+a,"i")):e(/iP(hone|ad|od)/i)};f.isAndroid=function(a,b){return b&&e(/chrome\/[123456789]/i)&&!e(/chrome\/18/)?!1:a?(f.isInt(a)&&!/\./.test(a)&&(a=""+a+"."),e(new RegExp("Android\\s*"+a,"i"))):e(/Android/i)};f.isMobile=function(){return f.isIOS()||f.isAndroid()};f.getProtocol=function(a){var b="http";(a=a.match(/^([a-z]+):\/\//i))&&1<a.length&&(b=a[1]);return b};f.getFileName=function(a){var b="";
(a=a.match(/\/([a-z0-9\(\)\[\]\{\}\s\-_%]*(\.[a-z0-9]+)?)$/i))&&1<a.length&&(b=a[1]);return b};f.getExtension=function(a){var b="";(a=a.match(/\/?([a-z0-9\(\)\[\]\{\}\s\-_%]*(\.([a-z0-9]+))*)\??([a-z0-9\-_%&=]*)$/i))&&3<a.length&&(b=a[3]);return b};var h=window.console=window.console||{log:function(){}};f.log=function(){var a=Array.prototype.slice.call(arguments,0);"object"===f.typeOf(h.log)?h.log(a):h.log.apply(h,a)}})(chatease);
(function(c){var e=c.utils;c=e.crypt={};c.stringToByteArray=function(f){for(var e=[],a=0,b=0;b<f.length;b++){for(var d=f.charCodeAt(b);255<d;)e[a++]=d&255,d>>=8;e[a++]=d}return e};c.byteArrayToString=function(f){if(8192>=f.length)return String.fromCharCode.apply(null,f);for(var e="",a=0;a<f.length;a+=8192){var b=Array.slice(f,a,a+8192);e+=String.fromCharCode.apply(null,b)}return e};c.byteArrayToHex=function(e){return Array.map(e,function(e){e=e.toString(16);return 1<e.length?e:"0"+e}).join("")};c.hexToByteArray=
function(f){if(0!==f.length%2)return e.log("Key string length must be multiple of 2."),null;for(var c=[],a=0;a<f.length;a+=2)c.push(parseInt(f.substring(a,a+2),16));return c};c.stringToUTF8ByteArray=function(e){for(var f=[],a=0,b=0;b<e.length;b++){var d=e.charCodeAt(b);128>d?f[a++]=d:(2048>d?f[a++]=d>>6|192:(55296==(d&64512)&&b+1<e.length&&56320==(e.charCodeAt(b+1)&64512)?(d=65536+((d&1023)<<10)+(e.charCodeAt(++b)&1023),f[a++]=d>>18|240,f[a++]=d>>12&63|128):f[a++]=d>>12|224,f[a++]=d>>6&63|128),f[a++]=
d&63|128)}return f};c.UTF8ByteArrayToString=function(e){for(var f=[],a=0,b=0;a<e.length;){var d=e[a++];if(128>d)f[b++]=String.fromCharCode(d);else if(191<d&&224>d){var g=e[a++];f[b++]=String.fromCharCode((d&31)<<6|g&63)}else if(239<d&&365>d){g=e[a++];var c=e[a++],u=e[a++];d=((d&7)<<18|(g&63)<<12|(c&63)<<6|u&63)-65536;f[b++]=String.fromCharCode(55296+(d>>10));f[b++]=String.fromCharCode(56320+(d&1023))}else g=e[a++],c=e[a++],f[b++]=String.fromCharCode((d&15)<<12|(g&63)<<6|c&63)}return f.join("")};c.XORByteArray=
function(f,c){if(f.length!==c.length)e.log("XOR array lengths must match.");else{for(var a=[],b=0;b<f.length;b++)a.push(f[b]^c[b]);return a}}})(chatease);
(function(c){function e(){var a=document.createElement("style");a.type="text/css";document.getElementsByTagName("head")[0].appendChild(a);return a.sheet||a.styleSheet}var f=c.utils,h;(f.css=function(a,b){h||(h=e());var d="";f.foreach(b,function(a,b){d+=a+": "+b+"; "});try{h.insertRule?h.insertRule(a+" { "+d+"}",h.cssRules.length):h.addRule(a,d,h.rules.length)}catch(g){f.log("Failed to insert css rule: "+a)}}).style=function(a,b,d){if(void 0!==a&&null!==a)for(void 0===a.length&&(a=[a]),b=f.extend({},
b),d=0;d<a.length;d++){var e=a[d];void 0!==e&&null!==e&&f.foreach(b,function(a,b){a=a.split("-");for(var d=1;d<a.length;d++)a[d]=a[d].charAt(0).toUpperCase()+a[d].slice(1);a=a.join("");e.style[a]!==b&&(e.style[a]=b)})}}})(chatease);(function(c){var e=c.utils;e.filter=function(f){var c="";this.replace=function(b){return c&&a?b.replace(a,"**"):b};if("string"==e.typeOf(f)&&f){c=f;var a=new RegExp(c,"ig")}}})(chatease);
(function(c){c.events={ERROR:"error",CHATEASE_READY:"chateaseReady",CHATEASE_SETUP_ERROR:"chateaseSetupError",CHATEASE_RENDER_ERROR:"chateaseRenderError",CHATEASE_STATE:"chateaseState",CHATEASE_PROPERTY:"chateaseProperty",CHATEASE_CONNECT:"chateaseConnect",CHATEASE_INFO:"chateaseInfo",CHATEASE_MESSAGE:"chateaseMessage",CHATEASE_USER:"chateaseUser",CHATEASE_JOIN:"chateaseJoin",CHATEASE_LEFT:"chateaseLeft",CHATEASE_CTRL:"chateaseCtrl",CHATEASE_EXTERN:"chateaseExtern",CHATEASE_NICKCLICK:"chateaseNickClick",
CHATEASE_CLOSE:"chateaseClose",CHATEASE_VIEW_SEND:"chateaseViewSend",CHATEASE_VIEW_PROPERTY:"chateaseViewProperty",CHATEASE_VIEW_SHOWMORE:"chateaseViewShowMore",CHATEASE_VIEW_CLEARSCREEN:"chateaseViewClearScreen",CHATEASE_VIEW_NICKCLICK:"chateaseViewNickClick",CHATEASE_TIMER:"chateaseTimer",CHATEASE_TIMER_COMPLETE:"chateaseTimerComplete"}})(chatease);
(function(c){var e=c.utils;c.events.eventdispatcher=function(f){function h(a,b,f){if(a)for(var d=0;d<a.length;d++){var g=a[d];if(g){null!==g.count&&0===--g.count&&delete a[d];try{g.listener(b)}catch(w){e.log('Error handling "'+f+'" event listener ['+d+"]: "+w.toString(),g.listener,b)}}}}var a={},b=[];this.addEventListener=function(b,g,f){try{e.exists(a[b])||(a[b]=[]),"string"===e.typeOf(g)&&(g=(new Function("return "+g))()),a[b].push({listener:g,count:f||null})}catch(u){e.log("error",u)}return!1};
this.removeEventListener=function(b,g){if(a[b]){try{if(void 0===g){a[b]=[];return}var d;for(d=0;d<a[b].length;d++)if(a[b][d].listener.toString()===g.toString()){a[b].splice(d,1);break}}catch(u){e.log("error",u)}return!1}};this.addGlobalListener=function(a,g){try{"string"===e.typeOf(a)&&(a=(new Function("return "+a))()),b.push({listener:a,count:g||null})}catch(l){e.log("error",l)}return!1};this.removeGlobalListener=function(a){if(a){try{var d;for(d=b.length-1;0<=d;d--)b[d].listener.toString()===a.toString()&&
b.splice(d,1)}catch(l){e.log("error",l)}return!1}};this.dispatchEvent=function(d,g){g||(g={});e.extend(g,{id:f,version:c.version,type:d});"events"===c.debug&&e.log(d,g);h(a[d],g,d);h(b,g,d)}}})(chatease);
(function(c){var e=c.utils,f=c.events;e.timer=function(c,a){function b(){l++;d.dispatchEvent(f.CHATEASE_TIMER);0<d.repeatCount&&l>=d.repeatCount&&(d.stop(),d.dispatchEvent(f.CHATEASE_TIMER_COMPLETE))}var d=e.extend(this,new f.eventdispatcher("utils.timer")),g,l=0,u=!1;d.start=function(){!1===u&&(g=setInterval(b,d.delay),u=!0)};d.stop=function(){u&&(clearInterval(g),g=0,u=!1)};d.reset=function(){d.stop();l=0};d.currentCount=function(){return l};d.running=function(){return u};d.delay=c||50;d.repeatCount=
a||0}})(chatease);
(function(c){var e=c.utils,f=c.events,h={},a={onError:f.ERROR,onReady:f.CHATEASE_READY,onConnect:f.CHATEASE_CONNECT,onIdent:f.CHATEASE_INFO,onMessage:f.CHATEASE_MESSAGE,onJoin:f.CHATEASE_JOIN,onLeft:f.CHATEASE_LEFT,onUser:f.CHATEASE_USER,onExtern:f.CHATEASE_EXTERN,onNickClick:f.CHATEASE_VIEW_NICKCLICK,onClose:f.CHATEASE_CLOSE};c.api=function(b){function d(a){g.dispatchEvent(a.type,a)}var g=e.extend(this,new f.eventdispatcher("api")),l;g.container=b;g.id=b.id;g.setup=function(a){e.emptyElement(g.container);c.debug=
!!a.debug;g.config=a;g.config.id=g.id;g.embedder=new c.embed(g);g.embedder.addGlobalListener(d);g.embedder.embed();return g};g.setEntity=function(a,b){l=a;g.onSWFLoaded=l.setup;g.onSWFOpen=l.onSWFOpen;g.onSWFMessage=l.onSWFMessage;g.onSWFError=l.onSWFError;g.onSWFClose=l.onSWFClose;g.send=l.send;g.close=l.close;g.getState=l.getState;g.resize=l.resize};(function(){e.foreach(a,function(a,b){g[a]=function(a){g.addEventListener(b,a)}})})()};c.api.getInstance=function(a){var b;null==a?a=0:a.nodeType?b=
a:"string"===e.typeOf(a)&&(b=document.getElementById(a));if(b){var f=h[b.id];f||(h[a]=f=new c.api(b));return f}return"number"===e.typeOf(a)?h[a]:null};c.api.displayError=function(a,d){}})(chatease);(function(c){c.core={}})(chatease);(function(c){c.core.states={CONNECTED:"connected",CLOSED:"closed",ERROR:"error"}})(chatease);
(function(c){c.core.message={};c=c.core.message;c.keys={CMD:"cmd",SEQ:"seq",DATA:"data",MODE:"mode",USER:"user",SUB:"sub",CHANNEL:"channel",GROUP:"group",ERROR:"error",ID:"id",NAME:"name",ICON:"icon",ROLE:"role",STAT:"stat",STATUS:"status",CODE:"code"};c.cmds={INFO:"info",TEXT:"text",USER:"user",JOIN:"join",LEFT:"left",CTRL:"ctrl",EXTERN:"extern",PING:"ping",PONG:"pong",ERROR:"error"};c.opts={MUTE:"mute",FORBID:"forbid"};c.modes={UNI:0,MULTI:1,BROADCAST:2,OUTDATED:4};c.roles={VISITOR:0,NORMAL:1,VIP:14,
ASSISTANT:16,SECRETARY:32,ANCHOR:48,ADMIN:64,SU_ADMIN:128,SYSTEM:192};c.status={BadRequest:400,Unauthorized:401,Forbidden:403,NotFound:404,MethodNotAllowed:405,NotAcceptable:406,ProxyAuthRequired:407,RequestTimeout:408,TooManyRequests:429,InternalServerError:500,NotImplemented:501,BadGateway:502,ServiceUnavailable:503,GatewayTimeout:504};c.parseControl=function(e){e=/^([a-z]+):([0-9]+)$/i.exec(e);return null==e?null:[e[1],parseInt(e[2])]}})(chatease);
(function(c){(c.core.skins={}).modes={DEFAULT:"def",MOBILE:"mobile"}})(chatease);
(function(c){var e=c.utils,f=c.events;c=c.core.skins;var h=c.modes,a=e.css,b="cha-skin";c.def=function(d){var c=e.extend(this,new f.eventdispatcher("skins.def"));c.resize=function(b,d){b=document.getElementById(c.config.id);d=document.getElementById(c.config.id+"-input");a.style(d,{width:b.clientWidth-90+"px"})};c.name=h.DEFAULT;c.config=e.extend({},d);b+="-"+c.name;a(".cha-wrapper",{width:"100%",height:"100%","background-color":"#171717"});a(".cha-wrapper *",{margin:"0",padding:"0","font-family":"Microsoft YaHei,arial,sans-serif",
"font-size":"14px","font-weight":"normal","box-sizing":"content-box"});a("."+b+" .cha-checkbox",{color:"#E6E6E6",cursor:"pointer"});a("."+b+" .cha-checkbox .icon",{"float":"left",padding:"0 5px",width:"14px",height:"14px","background-repeat":"no-repeat","background-position":"center","background-image":"url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA4AAAAOAgMAAABiJsVCAAAABGdBTUEAALGPC/xhBQAAAAFzUkdCAK7OHOkAAAAJUExURUxpcebm5ubm5kbBEu0AAAACdFJOUwCgoEVu0AAAABpJREFUCNdjYAABqVWrHBi0Vq1qII0AawMBACnPF0kf/g8sAAAAAElFTkSuQmCC)",
display:"inline-block"});a("."+b+" .cha-checkbox.checked .icon",{"background-image":"url(data:image/png;base64,)"});a("."+
b+" .cha-button",{padding:"0 10px","font-style":"normal","font-weight":"normal","text-align":"center","text-transform":"uppercase","white-space":"nowrap","word-spacing":"normal",border:"none","border-radius":"2px",cursor:"pointer",display:"inline-block","-webkit-font-smoothing":"subpixel-antialiased","-moz-osx-font-smoothing":"grayscale",transition:"150ms ease-in-out","transition-property":"background-color, color"});a("."+b+" .cha-button.red",{color:"#FFFFFF","background-color":"#FF0046"});a("."+
b+" .cha-button.red:hover",{"background-color":"#97052D"});a("."+b+" .cha-button.blue",{color:"#FFFFFF","background-color":"#0B7EF4"});a("."+b+" .cha-button.blue:hover",{"background-color":"#0966C3"});a("."+b+" .cha-button.white",{color:"#000000","background-color":"#FFFFFF"});a("."+b+" .cha-button.white:hover",{"background-color":"#E0E0E0"});a("."+b+" .cha-button.gray",{color:"#000000","background-color":"#E0E0E0"});a("."+b+" .cha-button.gray:hover",{"background-color":"#96A0B4"});a("."+b+" .cha-render",
{width:"100%",height:"100%",position:"relative"});a("."+b+" .cha-render object",{width:"0",height:"0",position:"absolute"});a("."+b+" .cha-render .cha-title",{width:"100%",height:"40px","font-family":"inherit","font-weight":"normal","text-align":"center","line-height":"40px",color:"#E6E6E6","background-color":"inherit",cursor:"default","pointer-events":"none"});a("."+b+" .cha-render .cha-console",{color:"#242424",top:(c.config.title?40:0)+"px",right:"0",bottom:"100px",left:"0",position:"absolute",
"background-color":"#F8F8F8","overflow-x":"hidden","overflow-y":c.config.smoothing?"hidden":"scroll"});a("."+b+" .cha-render .cha-console > div",{padding:"0 6px "+(c.config.smoothing?"6px":"0")+" 6px"});a("."+b+" .cha-render .cha-console > div > div",{margin:"6px 0","line-height":"20px"});a("."+b+" .cha-render .cha-console .cha-system, ."+b+" .cha-render .cha-console .cha-system > a",{"font-style":"normal","font-weight":"normal",color:"#33CC00",cursor:"default","pointer-events":"none"});a("."+b+" .cha-render .cha-console .icon",
{"float":"left","margin-right":"4px",width:"20px",height:"20px",display:"block"});a("."+b+" .cha-render .cha-console .icon img",{width:"100%",height:"100%",display:"inline-block"});a("."+b+" .cha-render .cha-console .area",{"margin-right":"2px"});a("."+b+" .cha-render .cha-console .area.area-uni",{color:"#F76767"});a("."+b+" .cha-render .cha-console .title",{"margin-right":"2px",padding:"0 2px",font:"normal 12px Microsoft YaHei,arial,sans-serif","white-space":"nowrap","word-spacing":"normal",border:"1px solid #3CAFAB",
"border-radius":"2px","vertical-align":"middle"});a("."+b+" .cha-render .cha-console .title.ttl-visitor, ."+b+" .cha-render .cha-console .title.ttl-normal, ."+b+" .cha-render .cha-console .title.ttl-system",{display:"none"});a("."+b+" .cha-render .cha-console .title.ttl-vip1, ."+b+" .cha-render .cha-console .title.ttl-vip2, ."+b+" .cha-render .cha-console .title.ttl-vip3",{color:"#3CAFAB","border-color":"#3CAFAB"});a("."+b+" .cha-render .cha-console .title.ttl-vip4, ."+b+" .cha-render .cha-console .title.ttl-vip5, ."+
b+" .cha-render .cha-console .title.ttl-vip6, ."+b+" .cha-render .cha-console .title.ttl-vip7",{color:"#77C773","border-color":"#77C773"});a("."+b+" .cha-render .cha-console .title.ttl-assistant, ."+b+" .cha-render .cha-console .title.ttl-secretary, ."+b+" .cha-render .cha-console .title.ttl-anchor",{color:"#5382E2","border-color":"#5382E2"});a("."+b+" .cha-render .cha-console .title.ttl-admin, ."+b+" .cha-render .cha-console .title.ttl-suadmin",{color:"#F76767","border-color":"#F76767"});a("."+b+
" .cha-render .cha-console a",{color:"#5382E2","text-decoration":"none",cursor:"pointer"});a("."+b+" .cha-render .cha-console .context",{"word-break":"break-all","word-wrap":"break-word"});a("."+b+" .cha-render .cha-controls",{height:"40px",right:"0",bottom:"60px",left:"0",position:"absolute",overflow:"hidden","line-height":"100%","background-color":"inherit"});a("."+b+" .cha-render .cha-controls > *",{margin:"8px 0","text-align":"center","line-height":"24px",display:"block"});a("."+b+" .cha-render .cha-controls .hidetext",
{"float":"left"});a("."+b+" .cha-render .cha-controls .hidetext .icon",{padding:"5px"});a("."+b+" .cha-render .cha-controls .clearscreen",{"float":"right",width:"50px"});a("."+b+" .cha-render .cha-dialog",{height:"60px",right:"0",bottom:"0",left:"0",position:"absolute",overflow:"hidden","background-color":"#E0E0E0"});a("."+b+" .cha-render .cha-dialog .more",{display:"none"});a("."+b+" .cha-render .cha-dialog textarea",{padding:"6px 10px",width:c.config.width-90+"px",height:"48px",resize:"none",border:"0 none",
"background-color":"#E6E6E6"});a("."+b+" .cha-render .cha-dialog .send",{width:"50px",height:"100%",top:"0",right:"0",position:"absolute"})}})(chatease);
(function(c){var e=c.utils,f=c.events;c=c.core.skins;var h=c.modes,a=e.css,b="cha-skin";c.mobile=function(d){var c=e.extend(this,new f.eventdispatcher("skins.mobile"));c.resize=function(b,d){b=document.getElementById(c.config.id);d=document.getElementById(c.config.id+"-input");a.style(d,{width:b.clientWidth-114+"px"})};c.name=h.MOBILE;c.config=e.extend({},d);b+="-"+c.name;a(".cha-wrapper",{width:"100%",height:"100%","background-color":"#171717"});a(".cha-wrapper *",{margin:"0",padding:"0","font-family":"Microsoft YaHei,arial,sans-serif",
"font-size":"14px","font-weight":"normal","box-sizing":"content-box"});a("."+b+" .cha-checkbox",{color:"#E6E6E6",cursor:"pointer"});a("."+b+" .cha-checkbox .icon",{"float":"left",padding:"0 5px",width:"14px",height:"14px","background-repeat":"no-repeat","background-position":"center","background-image":"url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAA4AAAAOAgMAAABiJsVCAAAABGdBTUEAALGPC/xhBQAAAAFzUkdCAK7OHOkAAAAJUExURUxpcebm5ubm5kbBEu0AAAACdFJOUwCgoEVu0AAAABpJREFUCNdjYAABqVWrHBi0Vq1qII0AawMBACnPF0kf/g8sAAAAAElFTkSuQmCC)",
display:"inline-block"});a("."+b+" .cha-checkbox.checked .icon",{"background-image":"url(data:image/png;base64,)"});a("."+
b+" .cha-button",{padding:"0 10px","font-style":"normal","font-weight":"normal","text-align":"center","text-transform":"uppercase","white-space":"nowrap","word-spacing":"normal",border:"none","border-radius":"2px",cursor:"pointer",display:"inline-block","-webkit-font-smoothing":"subpixel-antialiased","-moz-osx-font-smoothing":"grayscale",transition:"150ms ease-in-out","transition-property":"background-color, color"});a("."+b+" .cha-button.red",{color:"#FFFFFF","background-color":"#FF0046"});a("."+
b+" .cha-button.red:hover",{"background-color":"#97052D"});a("."+b+" .cha-button.blue",{color:"#FFFFFF","background-color":"#0B7EF4"});a("."+b+" .cha-button.blue:hover",{"background-color":"#0966C3"});a("."+b+" .cha-button.white",{color:"#000000","background-color":"#FFFFFF"});a("."+b+" .cha-button.white:hover",{"background-color":"#E0E0E0"});a("."+b+" .cha-button.gray",{color:"#000000","background-color":"#E0E0E0"});a("."+b+" .cha-button.gray:hover",{"background-color":"#96A0B4"});a("."+b+" .cha-render",
{width:"100%",height:"100%",position:"relative",overflow:"hidden"});a("."+b+" .cha-render object",{width:"0",height:"0",position:"absolute"});a("."+b+" .cha-render .cha-title",{width:"100%",height:"40px","font-family":"inherit","font-weight":"normal","text-align":"center","line-height":"40px",color:"#E6E6E6","background-color":"inherit",cursor:"default","pointer-events":"none"});a("."+b+" .cha-render .cha-console",{color:"#242424",top:(c.config.title?40:0)+"px",right:"0",bottom:"40px",left:"0",position:"absolute",
"background-color":"#F8F8F8","overflow-x":"hidden","overflow-y":c.config.smoothing?"hidden":"scroll"});a("."+b+" .cha-render.more .cha-console",{bottom:"130px"});a("."+b+" .cha-render .cha-console > div",{padding:"0 6px "+(c.config.smoothing?"6px":"0")+" 6px"});a("."+b+" .cha-render .cha-console > div > div",{margin:"6px 0","line-height":"20px"});a("."+b+" .cha-render .cha-console .cha-system, ."+b+" .cha-render .cha-console .cha-system > a",{"font-style":"normal","font-weight":"normal",color:"#33CC00",
cursor:"default","pointer-events":"none"});a("."+b+" .cha-render .cha-console .icon",{"float":"left","margin-right":"4px",width:"20px",height:"20px",display:"block"});a("."+b+" .cha-render .cha-console .icon img",{width:"100%",height:"100%",display:"inline-block"});a("."+b+" .cha-render .cha-console .area",{"margin-right":"2px"});a("."+b+" .cha-render .cha-console .area.area-uni",{color:"#F76767"});a("."+b+" .cha-render .cha-console .title",{"margin-right":"2px",padding:"0 2px",font:"normal 12px Microsoft YaHei,arial,sans-serif",
"white-space":"nowrap","word-spacing":"normal",border:"1px solid #3CAFAB","border-radius":"2px","vertical-align":"middle"});a("."+b+" .cha-render .cha-console .title.ttl-visitor, ."+b+" .cha-render .cha-console .title.ttl-normal, ."+b+" .cha-render .cha-console .title.ttl-system",{display:"none"});a("."+b+" .cha-render .cha-console .title.ttl-vip1, ."+b+" .cha-render .cha-console .title.ttl-vip2, ."+b+" .cha-render .cha-console .title.ttl-vip3",{color:"#3CAFAB","border-color":"#3CAFAB"});a("."+b+
" .cha-render .cha-console .title.ttl-vip4, ."+b+" .cha-render .cha-console .title.ttl-vip5, ."+b+" .cha-render .cha-console .title.ttl-vip6, ."+b+" .cha-render .cha-console .title.ttl-vip7",{color:"#77C773","border-color":"#77C773"});a("."+b+" .cha-render .cha-console .title.ttl-assistant, ."+b+" .cha-render .cha-console .title.ttl-secretary, ."+b+" .cha-render .cha-console .title.ttl-anchor",{color:"#5382E2","border-color":"#5382E2"});a("."+b+" .cha-render .cha-console .title.ttl-admin, ."+b+" .cha-render .cha-console .title.ttl-suadmin",
{color:"#F76767","border-color":"#F76767"});a("."+b+" .cha-render .cha-console a",{color:"#5382E2","text-decoration":"none",cursor:"pointer"});a("."+b+" .cha-render .cha-console .context",{"word-break":"break-all","word-wrap":"break-word"});a("."+b+" .cha-render .cha-controls",{margin:"0",padding:"0",height:"90px",bottom:"-90px",right:"0",left:"0",position:"absolute",overflow:"hidden","background-color":"#F8F8F8"});a("."+b+" .cha-render.more .cha-controls",{bottom:"0"});a("."+b+" .cha-render .cha-controls > *",
{"float":"left",margin:"8px 0 0 8px",padding:"0",width:"60px",color:"#8A8A8A","font-size":"12px","text-align":"center","white-space":"normal","background-color":"transparent"});a("."+b+" .cha-render .cha-controls > *:hover",{"background-color":"transparent"});a("."+b+" .cha-render .cha-controls > * .icon",{"float":"none",margin:"0 4px",padding:"0",width:"50px",height:"50px",border:"1px solid #E6E6E6","border-radius":"5px","background-repeat":"no-repeat","background-position":"center",display:"inline-block"});
a("."+b+" .cha-render .cha-controls .hidetext .icon",{"background-image":"url(data:image/png;base64,)"});
a("."+b+" .cha-render .cha-controls .hidetext.checked .icon",{"background-image":"url(data:image/png;base64,)"});
a("."+b+" .cha-render .cha-controls .clearscreen .icon",{"background-image":"url(data:image/png;base64,)"});
a("."+b+" .cha-render .cha-dialog",{height:"40px",right:"0",bottom:"0",left:"0",position:"absolute","background-color":"#E0E0E0"});a("."+b+" .cha-render.more .cha-dialog",{bottom:"90px"});a("."+b+" .cha-render .cha-dialog .more",{"float":"left",margin:"8px 5px",padding:"0",width:"24px",height:"24px","background-repeat":"no-repeat","background-position":"center","background-image":"url(data:image/png;base64,)",
display:"block"});a("."+b+" .cha-render .cha-dialog textarea",{margin:"5px 0",padding:"5px",width:c.config.width-114+"px",height:"20px",resize:"none",border:"0 none","line-height":"20px","background-color":"#F8F8F8"});a("."+b+" .cha-render .cha-dialog .send",{"float":"right",margin:"5px",padding:"0 5px",width:"50px",height:"30px",top:"0",right:"0",position:"absolute"})}})(chatease);(function(c){(c.core.renders={}).modes={DEFAULT:"def",NONE:"none"}})(chatease);
(function(c){var e=c.utils,f=c.events;c=c.core;var h=c.message,a=h.modes,b=h.roles;c=c.renders;var d=c.modes,g=e.css;c.def=function(c,h){function l(a,b,d,c,f){var g=e.createElement("div",b);g.setAttribute("value",!!f);g.innerHTML='<span class="icon"></span>'+a;a=function(a,b){return function(d){d="true"==g.getAttribute("value")?!1:!0;g.setAttribute("value",d);d?e.addClass(g,"checked"):e.removeClass(g,"checked");k.dispatchEvent(a,e.extend({value:d},b))}}(d,c);try{g.addEventListener("click",a)}catch(q){g.attachEvent("onclick",
a)}return g}function u(a,b,d,c){b=e.createElement("div",b);b.innerHTML='<span class="icon"></span>'+a;a=function(a,b){return function(d){k.dispatchEvent(a,b)}}(d,c);try{b.addEventListener("click",a)}catch(p){b.attachEvent("onclick",a)}return b}function r(a){if(!a)return null;var b=e.createElement("span","icon");b.innerHTML='<img src="'+a+'">';return b}function x(a){var d="title ";if("number"!=e.typeOf(a)&&(a=parseInt(a),NaN==a||0>a))return null;if(a&b.SYSTEM){if((a&b.SYSTEM)!=b.SYSTEM)if(a&b.SU_ADMIN){var c=
e.createElement("span",d+"ttl-suadmin");c.innerText="\u8d85\u7ba1"}else c=e.createElement("span",d+"ttl-admin"),c.innerText="\u7ba1\u7406\u5458"}else a&b.ANCHOR?(a&b.ANCHOR)==b.ANCHOR?(c=e.createElement("span",d+"ttl-anchor"),c.innerText="\u4e3b\u64ad"):a&b.SECRETARY?(c=e.createElement("span",d+"ttl-secretary"),c.innerText="\u79d8\u4e66"):(c=e.createElement("span",d+"ttl-assistant"),c.innerText="\u52a9\u7406"):a&b.VIP&&(a=(a&b.VIP)>>>1,c=e.createElement("span",d+("ttl-vip"+a)),c.innerText="VIP"+a);
return c}var k=e.extend(this,new f.eventdispatcher("renders.def")),y,v,n,B,C,z,t,E,m,A,F;k.setup=function(){e.isMSIE("(8|9)")?_object.setup&&(_object.setup(k.config),k.dispatchEvent(f.CHATEASE_READY,{id:k.config.id})):k.dispatchEvent(f.CHATEASE_READY,{id:k.config.id})};k.show=function(d,c,g){"object"!=e.typeOf(c)&&(c={id:0,name:"\u7cfb\u7edf",role:b.SYSTEM});var l=e.createElement("div");(c.role&b.SYSTEM)==b.SYSTEM&&(l.className="cha-system");var p=r(c.icon);p&&l.appendChild(p);g==a.UNI&&(p=e.createElement("span",
"area area-uni"),p.innerHTML="[\u5bc6\u8bed]",l.appendChild(p));(p=x(c.role))&&l.appendChild(p);p=e.createElement("a");p.innerHTML=c.name+": ";c=function(a){return function(b){m.value="/r "+a.name+" ";k.dispatchEvent(f.CHATEASE_VIEW_NICKCLICK,{user:a})}}(c);try{p.addEventListener("click",c)}catch(M){p.attachEvent("onclick",c)}l.appendChild(p);p=e.createElement("span","context");p.innerHTML=d;l.appendChild(p);n.childNodes.length>=k.config.maxrecords&&n.removeChild(n.childNodes[0]);g==a.OUTDATED?n.insertBefore(l,
n.childNodes[0]):n.appendChild(l);k.config.smoothing?k.refresh():v.scrollTop=v.scrollHeight};k.send=function(){k.dispatchEvent(f.CHATEASE_VIEW_SEND,{data:{data:m.value,mode:a.MULTI}});k.clearInput()};k.refresh=function(){k.config.smoothing?(v.clientHeight<n.clientHeight&&g.style(n,{"transition-timing-function":"cubic-bezier(0.165, 0.84, 0.44, 1)","transition-duration":"100ms",transform:"translate(0px, "+(v.clientHeight-n.clientHeight)+"px) translateZ(0px)"}),F.refresh()):v.scrollTop=v.scrollHeight};
k.clearInput=function(){m.value=""};k.clearScreen=function(){e.emptyElement(n)};k.element=function(){return _object};k.resize=function(a,b){};k.destroy=function(){};k.name=d.DEFAULT;k.config=e.extend({},{},h);e.isMSIE("(8|9)")&&(c.innerHTML='<object id="cha-swf" name="cha-swf" align="middle" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"><param name="movie" value="'+k.config.swf+'"><param name="quality" value="high"><param name="bgcolor" value="#ffffff"><param name="allowscriptaccess" value="sameDomain"><param name="allowfullscreen" value="true"><param name="wmode" value="transparent"><param name="FlashVars" value="id='+
k.config.id+'"></object>',_object=k.WebSocket=c.firstChild);(function(){k.config.title&&(y=e.createElement("div","cha-title"),c.appendChild(y),y.innerHTML=k.config.title);v=e.createElement("div","cha-console");v.id=k.config.id+"-console";c.appendChild(v);n=e.createElement("div","cha-content");v.appendChild(n);k.config.smoothing&&(F=new BScroll(v));B=e.createElement("div","cha-controls");c.appendChild(B);z=l("\u5c4f\u853d\u6d88\u606f","cha-checkbox hidetext",f.CHATEASE_VIEW_PROPERTY,{key:"hidden"},
!1);z.id=k.config.id+"-hidetext";B.appendChild(z);t=u("\u6e05\u5c4f","cha-button white clearscreen",f.CHATEASE_VIEW_CLEARSCREEN,null);B.appendChild(t);C=e.createElement("div","cha-dialog");c.appendChild(C);E=l("","more",f.CHATEASE_VIEW_PROPERTY,{key:"more"},!1);C.appendChild(E);m=e.createElement("textarea");m.id=k.config.id+"-input";C.appendChild(m);A=e.createElement("button","cha-button blue send");C.appendChild(A);m.setAttribute("placeholder","\u8f93\u5165\u804a\u5929\u5185\u5bb9");k.config.maxlength&&
m.setAttribute("maxlength",k.config.maxlength);var a=function(){return function(a){a=window.event||a;13==a.keyCode&&(a.ctrlKey?m.value+="\r\n":(k.send(),window.event?a.returnValue=!1:a.preventDefault()))}}();try{m.addEventListener("keypress",a)}catch(D){m.attachEvent("onkeypress",a)}A.innerHTML="\u53d1\u9001";a=function(){return function(){k.send()}}();try{A.addEventListener("click",a)}catch(D){A.attachEvent("onclick",a)}})()}})(chatease);
(function(c){var e=c.utils,f=c.events;c=c.core.renders;var h=c.modes;c.none=function(a,b){var d=e.extend(this,new f.eventdispatcher("renders.none"));d.setup=function(){e.isMSIE("(8|9)")?_object.setup&&(d.config.debug=!0,_object.setup(d.config),d.dispatchEvent(f.CHATEASE_READY,{id:d.config.id})):d.dispatchEvent(f.CHATEASE_READY,{id:d.config.id})};d.show=function(a,b,d){};d.send=function(){};d.clearInput=function(){};d.clearScreen=function(){};d.element=function(){return _object};d.resize=function(a,
b){};d.destroy=function(){};d.name=h.NONE;d.config=e.extend({},{},b);e.isMSIE("(8|9)")&&(a.innerHTML='<object id="cha-swf" name="cha-swf" align="middle" classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"><param name="movie" value="'+d.config.swf+'"><param name="quality" value="high"><param name="bgcolor" value="#ffffff"><param name="allowscriptaccess" value="sameDomain"><param name="allowfullscreen" value="true"><param name="wmode" value="transparent"><param name="FlashVars" value="id='+d.config.id+
'"></object>',_object=d.WebSocket=a.firstChild)}})(chatease);
(function(c){var e=c.utils,f=c.events,h=c.core;h.entity=function(a){var b=e.extend(this,new f.eventdispatcher("core.entity")),d,c,l;b.setup=function(){setTimeout(function(){l.setup()},0)};b.destroy=function(){l&&l.stop();c&&c.destroy();d&&d.destroy()};b.id=a.id;b.model=d=new h.model(a);b.view=c=new h.view(d);b.controller=l=new h.controller(d,c);l.addGlobalListener(function(a){b.dispatchEvent(a.type,a)});b.onSWFOpen=l.onOpen;b.onSWFMessage=l.onMessage;b.onSWFError=l.onError;b.onSWFClose=l.onClose;
b.send=l.send;b.close=l.close;b.getState=d.getState;b.resize=c.resize}})(chatease);
(function(c){var e=c.utils,f=c.events;c=c.core;var h=c.states,a=function(){var a=this,b;a.update=function(b){e.extend(a,b);b=a.role;if(b&240)c=0;else{var d=2E3;b&&(d=1E3-100*(b>>1));c=d}};a.limited=function(){var a=(new Date).getTime();if(a<b+c)return!0;b=a;return!1};e.extend(a,{id:0,name:"",icon:"",role:0});var c=b=0},b=function(){var a=this;a.update=function(b){e.extend(a,b)};a.add=function(a){b[a.id]=a};a.remove=function(a){delete b[a.id]};a.find=function(a){return b[a]};a.freeze=function(a,b,
d,e){var f=c[a];void 0==f&&(f={},c[a]=f);f[b]={manager:d,until:Math.floor((new Date).getTime()/1E3)+e}};a.unfreeze=function(a,b){(a=c[a])&&delete a[b]};a.limited=function(a,b){if(a=c[a])if(b=a[b])return Math.floor((new Date).getTime()/1E3)<b.until;return!1};e.extend(a,{id:0,stat:0,group:0});var b={};var c={}};c.model=function(c){var d=e.extend(this,new f.eventdispatcher("core.model")),l=h.CLOSED;d.setState=function(a){a!==l&&(l=a,d.dispatchEvent(f.CHATEASE_STATE,{state:a}))};d.getState=function(){return l};
d.setProperty=function(a,b){1==u.hasOwnProperty(a)&&(u[a]=b,d.dispatchEvent(f.CHATEASE_PROPERTY,{key:a,value:b}))};d.getProperty=function(a){return u[a]};d.getConfig=function(a){return d.config[a]};d.destroy=function(){};d.config=e.extend({},{},c);var u={user:new a,channel:new b,hidden:!1}}})(chatease);
(function(c){var e=c.utils,f=c.events;c=c.core;var h=c.message.roles,a=c.renders,b=a.modes,d=c.skins;c.view=function(c){function g(a){if(a.ctrlKey||a.metaKey)return!0;switch(a.keyCode){case 13:n&&n.send()}if(/13/.test(a.keyCode))return a.preventDefault?a.preventDefault():a.returnValue=!1,!1}function u(a){x.resize()}function G(a){switch(a.key){case "more":a.value?e.addClass(y,"more"):e.removeClass(y,"more");n.refresh();break;case "hidden":var b=a.value?"\u53d6\u6d88\u5c4f\u853d":"\u5c4f\u853d\u6d88\u606f";
document.getElementById(c.getConfig("id")+"-shieldtext").innerHTML='<span class="icon"></span>'+b}r(a)}function w(a){r(a)}function r(a){x.dispatchEvent(a.type,a)}var x=e.extend(this,new f.eventdispatcher("core.view")),k,y,v,n,B;x.setup=function(){n?(n.setup(),x.resize()):x.dispatchEvent(f.CHATEASE_SETUP_ERROR,{message:"Render not available!",name:c.config.render.name})};x.show=function(a,b,d){(!b||b.role&h.SYSTEM||!c.getProperty("hidden"))&&n&&n.show(a,b,d)};x.resize=function(a,b){setTimeout(function(){n&&
n.resize(a,b);B&&B.resize(a,b)},0)};x.destroy=function(){k&&k.removeEventListener("keydown",g);n&&n.destroy()};(function(){k=e.createElement("div","cha-wrapper cha-skin-"+c.getConfig("skin").name);k.id=c.getConfig("id");y=e.createElement("div","cha-render");v=e.createElement("div","cha-contextmenu");k.appendChild(y);k.appendChild(v);var h=e.extend({},c.getConfig("render"),{id:c.getConfig("id"),url:c.getConfig("url"),width:c.getConfig("width"),height:c.getConfig("height"),maxlength:c.getConfig("maxlength"),
maxrecords:c.getConfig("maxrecords"),smoothing:c.getConfig("smoothing")});try{n=x.render=new a[h.name](y,h),n.addEventListener(f.CHATEASE_READY,r),n.addEventListener(f.CHATEASE_VIEW_SEND,r),n.addEventListener(f.CHATEASE_VIEW_PROPERTY,G),n.addEventListener(f.CHATEASE_VIEW_CLEARSCREEN,r),n.addEventListener(f.CHATEASE_VIEW_NICKCLICK,r),n.addEventListener(f.CHATEASE_RENDER_ERROR,w)}catch(z){e.log("Failed to init render "+h.name+"!")}h=e.extend({},c.getConfig("skin"),{id:c.getConfig("id"),width:c.config.width,
height:c.config.height,title:!!c.getConfig("render").title,smoothing:c.getConfig("smoothing")});try{B=new d[h.name](h)}catch(z){e.log("Failed to init skin "+h.name+"!")}if(!n||n.name!=b.NONE){h=document.getElementById(c.getConfig("id"));h.parentNode.replaceChild(k,h);try{k.addEventListener("keydown",g),window.addEventListener("resize",u)}catch(z){k.attachEvent("onkeydown",g),window.attachEvent("onresize",u)}}})()}})(chatease);
(function(c){var e=c.utils,f=e.crypt,h=c.events,a=c.core,b=a.states,d=a.message,g=d.keys,l=d.cmds,u=d.opts,G=d.modes,w=d.roles,r=d.status;a.controller=function(a,k){function y(){if(m){if(a.getState()==b.CONNECTED){e.log("Websocket had connected already.");return}m.readyState!=WebSocket.CONNECTING&&m.readyState!=WebSocket.OPEN||m.close()}k.render.clearScreen();c.debug?k.show("\u804a\u5929\u5ba4\u8fde\u63a5\u4e2d\u2026"):e.log("\u804a\u5929\u5ba4\u8fde\u63a5\u4e2d\u2026");try{window.WebSocket=window.WebSocket||
window.MozWebSocket,window.WebSocket?(m=new WebSocket(a.config.url,"binary"),m.binaryType="arraybuffer",m.onopen=t.onOpen,m.onmessage=t.onMessage,m.onerror=t.onError,m.onclose=t.onClose):(m=k.render.WebSocket,m.connect())}catch(p){e.log("Failed to initialize websocket: "+p),n(r.NOT_ACCEPTABLE,null)}}function v(a){var b="";if("number"!=e.typeOf(a)&&(a=parseInt(a),NaN==a||0>a))return"";a&w.SYSTEM?b=(a&w.SYSTEM)==w.SYSTEM?b+"[\u7cfb\u7edf] ":a&w.SU_ADMIN?b+"[\u8d85\u7ba1] ":b+"[\u7ba1\u7406\u5458] ":
a&w.ANCHOR?b=(a&w.ANCHOR)==w.ANCHOR?b+"[\u4e3b\u64ad] ":a&w.SECRETARY?b+"[\u52a9\u7406] ":b+"[\u623f\u7ba1] ":a&w.VIP?b+="[VIP"+((a&w.VIP)-1)+"] ":(a&w.NORMAL)!=w.NORMAL&&(b+="[\u6e38\u5ba2] ");return b}function n(a,b){var c=x(a);c&&k.show(c);var d={raw:"error",error:{code:a,status:c}};e.foreach(b,function(a,b){"cmd"!=a&&"raw"!=a&&"error"!=a&&(d[a]=b)});t.dispatchEvent(h.ERROR,d)}function x(a){var b="";switch(a){case r.BadRequest:b="\u9519\u8bef\u8bf7\u6c42\uff01";break;case r.Unauthorized:b="\u8bf7\u5148\u767b\u5f55\uff01";
break;case r.Forbidden:b="\u6743\u9650\u9519\u8bef\uff01";break;case r.NotFound:b="\u672a\u77e5\u8bf7\u6c42\uff01";break;case r.NotAcceptable:b="\u65e0\u6cd5\u8bc6\u522b\uff01";break;case r.RequestTimeout:b="\u8bf7\u6c42\u8d85\u65f6\uff01";break;case r.TooManyRequests:b="\u64cd\u4f5c\u9891\u7e41\uff01";break;case r.InternalServerError:b="\u5185\u90e8\u9519\u8bef\uff01";break;case r.NotImplemented:b="\u65e0\u6cd5\u8bc6\u522b\uff01";break;case r.BadGateway:b="\u7f51\u5173\u9519\u8bef\uff01";break;case r.ServiceUnavailable:b=
"\u670d\u52a1\u8fc7\u8f7d\uff01";break;case r.GatewayTimeout:b="\u7f51\u5173\u8d85\u65f6\uff01"}return b}function C(a){A||(A=new e.timer(a,1),A.addEventListener(h.CHATEASE_TIMER,function(a){y()}));A.delay=a;A.reset();A.start()}function z(a){t.dispatchEvent(a.type,a)}var t=e.extend(this,new h.eventdispatcher("core.controller")),E=!1,m,A,F,H=0;t.setup=function(a){E||k.setup()};t.send=function(c,d){if(m&&a.getState()==b.CONNECTED)if("object"!=e.typeOf(c)||0==c.hasOwnProperty(g.CMD)||0==c.hasOwnProperty(g.DATA))n(r.BadRequest,
c);else{var h=a.getProperty("user");if(h.limited())n(r.TooManyRequests,c);else{var k=a.getProperty("channel");if(h.role<k.stat||k.limited(h.id,u.MUTE))n(r.Forbidden,c);else{d&&(I++,D[I]=d,c.seq=I);c=JSON.stringify(c);try{var l=f.stringToUTF8ByteArray(c),p=new Uint8Array(l);m.send(p.buffer);return}catch(L){}try{m.send(c)}catch(L){}}}}else y()};t.onOpen=function(c){a.setState(b.CONNECTED)};t.onMessage=function(b){var p=b.data;"arraybuffer"===e.typeOf(b.data)&&(p=new Uint8Array(b.data),p=f.UTF8ByteArrayToString(p));
try{var q=eval("("+p+")")}catch(K){e.log("Failed to parse JSON. \nError: "+K+"\ndata: "+b.data);return}q.hasOwnProperty(g.SEQ)&&D.hasOwnProperty(q.seq)&&(b=D[q.seq],p=b.result,q.cmd==l.ERROR&&(p=b.status),p&&p.call(null,q),delete D[q.seq]);b=a.getProperty("user");p=a.getProperty("channel");switch(q.cmd){case l.INFO:b.update(e.extend({},q.user));p.update(e.extend({},q.channel));c.debug?k.show("\u5df2\u52a0\u5165\u623f\u95f4\uff08"+p.id+"\uff09\u3002"):e.log("\u5df2\u52a0\u5165\u623f\u95f4\uff08"+p.id+
"\uff09\u3002");b.role<p.stat&&k.show("\u60a8\u6240\u5728\u7684\u7528\u6237\u7ec4\u4e0d\u80fd\u53d1\u8a00\uff01");t.dispatchEvent(h.CHATEASE_INFO,q);break;case l.TEXT:try{F||(F=new e.filter(a.config.keywords)),q.data=F.replace(q.data)}catch(K){e.log("Failed to execute filter.")}p.add(q.user);k.show(q.data,q.user,q.mode);t.dispatchEvent(h.CHATEASE_MESSAGE,q);break;case l.USER:t.dispatchEvent(h.CHATEASE_USER,q);break;case l.JOIN:p.add(q.user);b=v(q.user.role);k.show(b+q.user.name+" \u8fdb\u5165\u623f\u95f4\u3002");
t.dispatchEvent(h.CHATEASE_JOIN,q);break;case l.LEFT:p.remove(q.user);b=v(q.user.role);k.show(b+q.user.name+" \u79bb\u5f00\u623f\u95f4\u3002");t.dispatchEvent(h.CHATEASE_LEFT,q);break;case l.CTRL:var m=d.parseControl(q.data);if(m){var r=m[0];m=m[1];0==m?p.unfreeze(q.sub,r):p.freeze(q.sub,r,q.user,m);if(q.sub==b.id)switch(r){case u.MUTE:k.show("\u60a8\u5df2\u88ab"+(m?"":"\u89e3\u9664")+"\u7981\u8a00"+(m?m+"\u79d2":"")+"\u3002");break;case u.FORBID:k.show("\u60a8\u5df2\u88ab\u9650\u5236\u8fdb\u5165\u8be5\u623f\u95f4"+
m+"\u79d2\u3002"),a.config.maxretries=0,t.close()}}break;case l.EXTERN:t.dispatchEvent(h.CHATEASE_EXTERN,q);break;case l.ERROR:n(q.error.code,q);break;default:e.log("Unknown cmd: "+q.cmd+", ignored.")}};t.onError=function(c){a.setState(b.ERROR)};t.onClose=function(c){a.setState(b.CLOSED)};t.close=function(){m&&m.close()};var D={};var I=0;var J="";a.addEventListener(h.CHATEASE_STATE,function(d){switch(d.state){case b.CONNECTED:c.debug?k.show("\u804a\u5929\u5ba4\u5df2\u8fde\u63a5\u2026"):e.log("\u804a\u5929\u5ba4\u5df2\u8fde\u63a5\u2026");
H=0;t.dispatchEvent(h.CHATEASE_CONNECT);break;case b.CLOSED:c.debug?k.show("\u804a\u5929\u5ba4\u8fde\u63a5\u5df2\u65ad\u5f00\uff01"):e.log("\u804a\u5929\u5ba4\u8fde\u63a5\u5df2\u65ad\u5f00\uff01");t.dispatchEvent(h.CHATEASE_CLOSE,{channel:{id:a.channel}});if(0>a.config.maxretries||H<a.config.maxretries)d=Math.ceil(a.config.retrydelay+3E3*Math.random()),c.debug?k.show("\u6b63\u5728\u51c6\u5907\u91cd\u8fde\uff0c"+d/1E3+"\u79d2..."):e.log("\u6b63\u5728\u51c6\u5907\u91cd\u8fde\uff0c"+d/1E3+"\u79d2..."),
H++,C(d);break;case b.ERROR:c.debug?k.show("\u804a\u5929\u5ba4\u5f02\u5e38\uff01"):e.log("\u804a\u5929\u5ba4\u5f02\u5e38\uff01");t.dispatchEvent(h.ERROR,{message:"Chat room error!",channel:{id:a.channel}});break;default:t.dispatchEvent(h.ERROR,{message:"Unknown model state!",state:d.state})}});a.addEventListener(h.CHATEASE_PROPERTY,function(a){z(a)});k.addEventListener(h.CHATEASE_READY,function(b){E||(e.log("Chat ready!"),E=!0,z(b),c.debug||k.show("\u804a\u5929\u5ba4\u5df2\u8fde\u63a5\uff01"),y(),
window.addEventListener("beforeunload",function(b){m&&a.getState()}))});k.addEventListener(h.CHATEASE_SETUP_ERROR,function(c){a.setState(b.ERROR);z(c)});k.addEventListener(h.CHATEASE_VIEW_SEND,function(b){var c=b.data.data;if(c){b={cmd:l.TEXT,data:c,mode:b.data.mode};var d=c.match(/^\/r\s(\S+)\s(.*)/i);if(d&&2<d.length){if(!a.getProperty("channel").find(J)){n(r.NotFound,null);return}b.mode=G.UNI;b.sub=J;c=d[2]}0<=a.config.maxlength&&(c=c.substr(0,a.config.maxlength));if(c=e.trim(c))b.data=c,t.send(b)}else k.show("\u8bf7\u8f93\u5165\u5185\u5bb9\uff01")});
k.addEventListener(h.CHATEASE_VIEW_PROPERTY,function(b){a.setProperty(b.key,b.value)});k.addEventListener(h.CHATEASE_VIEW_CLEARSCREEN,function(a){k.render.clearScreen();z(a)});k.addEventListener(h.CHATEASE_VIEW_NICKCLICK,function(b){var c=a.getProperty("channel");J=b.user.id;t.dispatchEvent(h.CHATEASE_NICKCLICK,{user:b.user,channel:{id:c.id,group:c.group,stat:c.stat}})});k.addEventListener(h.CHATEASE_RENDER_ERROR,function(c){a.setState(b.ERROR);z(c)})}})(chatease);
(function(c){var e=c.utils,f=c.events,h=c.embed=function(a){function b(a){switch(a.type){case f.ERROR:case f.CHATEASE_SETUP_ERROR:case f.CHATEASE_RENDER_ERROR:d.errorScreen(a.message);d.dispatchEvent(f.ERROR,a);break;default:d.dispatchEvent(a.type,a)}}var d=e.extend(this,new f.eventdispatcher("embed")),g={},l=null,u=!1;d.embed=function(){try{g=new h.config(a.config),l=new h.embedder(a,g)}catch(G){e.log("Failed to init embedder!");d.dispatchEvent(f.CHATEASE_SETUP_ERROR,{message:"Failed to init embedder!",
render:g.render.name});return}l.addGlobalListener(b);l.embed()};d.errorScreen=function(a){u||(u=!0,c.api.displayError(a,g))};(function(){e.foreach(a.config.events,function(b,c){b=a[b];"function"===e.typeOf(b)&&b.call(a,c)})})()}})(chatease);
(function(c){var e=c.utils,f=c.embed,h=c.core.renders.modes,a=c.core.skins.modes;f.config=function(b){b=e.extend({},{url:"ws://"+window.location.host+"/ch1?token=",width:640,height:400,keywords:"",maxlength:50,maxrecords:50,maxretries:-1,retrydelay:3E3,smoothing:!1,debug:!1,render:{name:h.DEFAULT,title:"CHATEASE "+c.version,swf:"swf/chatease.swf"},skin:{name:a.DEFAULT}},b);b.smoothing=b.smoothing&&e.isMobile()&&BScroll;return b};f.config.addConfig=function(a,c){return e.extend(a,c)}})(chatease);
(function(c){var e=c.utils,f=c.events,h=c.core;c.embed.embedder=function(a,b){function c(a){g.dispatchEvent(a.type,a)}var g=e.extend(this,new f.eventdispatcher("embed.embedder"));g.embed=function(){var d=new h.entity(b);d.addGlobalListener(c);d.setup();a.setEntity(d,b.render.name)}}})(chatease);